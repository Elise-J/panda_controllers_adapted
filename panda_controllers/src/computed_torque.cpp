...
namespace panda_controllers
{
	...
	bool ComputedTorque::init(hardware_interface::RobotHW *robot_hw, ros::NodeHandle &node_handle)
	{
		...
		if (!node_handle.getParam("kp1", kp1) || !node_handle.getParam("kp2", kp2) || !node_handle.getParam("kp3", kp3) || !node_handle.getParam("kp4", kp4) || !node_handle.getParam("kp5", kp5) || !node_handle.getParam("kp6", kp6) || !node_handle.getParam("kp7", kp7) || !node_handle.getParam("kv1", kv1) || !node_handle.getParam("kv2", kv2) || !node_handle.getParam("kv3", kv3) || !node_handle.getParam("kv4", kv4) || !node_handle.getParam("kv5", kv5) || !node_handle.getParam("kv6", kv6) || !node_handle.getParam("kv7", kv7))
		{
	...
	void ComputedTorque::update(const ros::Time &, const ros::Duration &period)
	{
		...
		if (torque_limit)
		{
			Eigen::Matrix<double, 7, 1> clip_tau{};
			for (size_t i = 0; i < 7; i++)
			{
				if (tau_cmd[i] >= 0)
				{
	...
	void ComputedTorque::setCommandParam(const panda_controllers::CommandParams &msg)
	{
		...
		{
			Kp(0, 0) = msg.stiffness[0];
			Kp(1, 1) = msg.stiffness[1];
			Kp(2, 2) = msg.stiffness[2];
			Kp(3, 3) = msg.stiffness[3];
			Kp(4, 4) = msg.stiffness[4];
			Kp(5, 5) = msg.stiffness[5];
			Kp(6, 6) = msg.stiffness[6];
		}

		if (!(msg.damping).empty())
		{
			Kv(0, 0) = msg.damping[0];
			Kv(1, 1) = msg.damping[1];
			Kv(2, 2) = msg.damping[2];
			Kv(3, 3) = msg.damping[3];
			Kv(4, 4) = msg.damping[4];
			Kv(5, 5) = msg.damping[5];
